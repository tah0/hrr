...
class TestVector(unittest.TestCase):
    ...
    def test_pow(self):
        ...
        hrr_res = hrr_vec**2
        np.testing.assert_allclose(np.array(hrr_res.values), np.fft.ifft(np.power(np.fft.fft(np_vec), 2)))        
    ...
    def test_mul(self):
        ...
        np.testing.assert_allclose(hrr_vec_A * hrr_vec_B, np.dot(np_vec_A, np_vec_B))        
    ...
    def test_truediv(self):
        ...
        np.testing.assert_allclose(np.array(hrr_res.values), np.true_divide(np_vec, divisor))
    ...
    def test_floordiv(self):
        ...
        divisor = np.random.uniform(0,1)
        hrr_res = hrr_vec // divisor
        np.testing.assert_allclose(np.array(hrr_res.values), np.floor_divide(np_vec, divisor))
